...
import { UserPlus, X, Eye, EyeOff, Pencil, Trash2, AlertTriangle, Mail, ChevronUp, ChevronDown, ChevronsUpDown } from 'lucide-vue-next'
...
const createUser = async () => {
  ...
  try {
    ...
    if (res.status === 401 || res.status === 403) { formError.value = 'Session expired. Please log in again.'; return }
    const result = await res.json()
    if (result.success) { users.value.unshift(result.data); closeModal(); showToast('User created successfully!') }
    else formError.value = result.message || 'Failed to create user'
...
const updateUser = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      closeModal(); showToast('User updated successfully!')
    } else formError.value = result.message || 'Failed to update user'
  } catch { formError.value = 'Network error. Please try again.' }
  finally { submitting.value = false }
}
...
const confirmDelete = async () => {
  submitting.value = true
  try {
    const res = await fetch(`${BASE_URL}/delete/${selectedUser.value.id}`, { method: 'DELETE', headers: authHeaders() })
    if (res.status === 401 || res.status === 403) { showToast('Session expired. Please log in again.', 'error'); return }
    const result = await res.json()
    if (result.success) {
      users.value = users.value.filter(u => u.id !== selectedUser.value.id)
      showDeleteModal.value = false; selectedUser.value = null
...
const openCreateModal = () => {
  isEditing.value = false; selectedUser.value = null
  form.value = { username: '', email: '', password: '' }
  formError.value = ''; showPassword.value = false; showModal.value = true
}
const openEditModal = (user) => {
  isEditing.value = true; selectedUser.value = user
...
const closeModal = () => { showModal.value = false; selectedUser.value = null; formError.value = '' }
...
<template>
  <!-- Sidebar: root sibling, never wrapped -->
  <DashboardSidebar
  ...
  <!-- Main content: bg + correct margin on same div -->
  <div
    :class="[
      'min-h-screen bg-gray-100/80 transition-all duration-300 ml-0',
    ...
    <main class="pt-3 px-3 sm:px-5 pb-8 mt-16">
      ...
      <div v-if="loading" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
        <div v-for="i in 10" :key="i"
          class="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5 animate-pulse flex flex-col items-center">
          <div class="w-14 h-14 sm:w-16 sm:h-16 bg-gray-200 rounded-xl mb-3"></div>
      ...
      <template v-else>
        ...
        <div v-if="filteredUsers.length === 0"
          class="text-center py-14 sm:py-16 text-gray-400 bg-white rounded-2xl border border-gray-100">
          <div class="w-14 h-14 sm:w-16 sm:h-16 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <svg class="w-7 h-7 sm:w-8 sm:h-8 opacity-40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-else-if="viewMode === 'grid'"
          ...
          <div v-for="item in paginatedUsers" :key="item.id"
            class="bg-white rounded-2xl shadow-sm border border-gray-100 p-3 sm:p-5 flex flex-col items-center text-center hover:shadow-md transition-all hover:-translate-y-0.5 relative group">

            <div class="absolute top-2.5 right-2.5 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
              ...
              <button @click="openDeleteModal(item)" title="Delete"
            ...
            <div class="relative mb-3">
              <div class="w-12 h-12 sm:w-16 sm:h-16 rounded-xl flex items-center justify-center text-white text-base sm:text-lg font-bold shadow-md"
                :style="{ backgroundColor: getColor(item.id) }">
                {{ getInitials(item.username) }}
              </div>
              <div class="absolute -bottom-1 -right-1 w-5 h-5 rounded-md flex items-center justify-center text-white text-xs font-bold shadow"
                :style="{ backgroundColor: getColor(item.id + 3) }">
                {{ getInitials(item.username).slice(0, 1) }}
              </div>
            </div>

            <h3 class="font-semibold text-gray-800 text-xs sm:text-sm leading-tight truncate w-full">{{ item.username }}</h3>
        ...
        <div v-else class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <thead>
                <tr class="border-b border-gray-100 bg-gray-50">
                  <th v-for="col in columns" :key="col.key"
                    :class="['px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide select-none',
                      col.class,
                      col.hideOnMobile ? 'hidden sm:table-cell' : '',
                      col.sortable ? 'cursor-pointer hover:text-gray-700 hover:bg-gray-100 transition' : '']"
                    @click="col.sortable ? handleSort(col.key) : null">
                    <div :class="['flex items-center gap-1', col.key === 'actions' ? 'justify-center' : '']">
                      <span>{{ col.label }}</span>
                      <span v-if="col.sortable" class="text-gray-300">
                        <ChevronUp v-if="sortKey === col.key && sortOrder === 'asc'" class="w-3 h-3 text-blue-500" />
                        <ChevronDown v-else-if="sortKey === col.key && sortOrder === 'desc'" class="w-3 h-3 text-blue-500" />
                        <ChevronsUpDown v-else class="w-3 h-3" />
                      </span>
                    </div>
                  </th>
                </tr>
                <tr v-if="showFilterRow" class="border-b border-gray-100 bg-white">
                  <td v-for="col in columns" :key="col.key"
                    :class="['px-3 py-2', col.hideOnMobile ? 'hidden sm:table-cell' : '']">
                    <input v-if="col.filterable"
                      :value="filters[col.key] || ''"
                      @input="updateFilter(col.key, $event.target.value)"
                      :placeholder="`Filter ${col.label}…`"
                      class="w-full text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-transparent placeholder:text-gray-300" />
                  </td>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-50">
                <tr v-for="row in paginatedUsers" :key="row.id" class="hover:bg-gray-50/70 transition-colors">
                  <td class="px-4 py-3 w-20">
                    <span class="text-gray-400 text-xs font-mono">#{{ row.id }}</span>
                  </td>
                  <td class="px-4 py-3">
                  ...
                  <td class="px-4 py-3 hidden sm:table-cell">
                    <span class="text-gray-600 text-sm">{{ row.email }}</span>
                  </td>
                  <td class="px-4 py-3 hidden sm:table-cell">
                    <span class="text-gray-400 text-xs">{{ formatDate(row.createdAt) }}</span>
                  </td>
          ...
          <div class="sm:hidden divide-y divide-gray-100">
            <div v-for="(row, idx) in paginatedUsers" :key="`m-${row.id}`"
              class="flex items-center gap-3 px-3 py-3"
              :class="idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'">
              <div class="w-10 h-10 rounded-xl flex items-center justify-center text-white text-sm font-bold shrink-0 shadow-sm"
                :style="{ backgroundColor: getColor(row.id) }">
                {{ getInitials(row.username) }}
              </div>
              <div class="flex-1 min-w-0">
                <p class="font-semibold text-gray-800 text-sm truncate">{{ row.username }}</p>
                <p class="text-xs text-gray-400 truncate">{{ row.email }}</p>
                <p class="text-xs text-gray-300 mt-0.5">{{ formatDate(row.createdAt) }}</p>
              </div>
          ...
          <div class="relative bg-white sm:rounded-2xl rounded-t-2xl shadow-2xl w-full sm:max-w-md z-10 overflow-hidden">
            ...
            <div class="px-5 py-5 space-y-4">
              ...
              <div>
                ...
                <input v-model="form.username" type="text" placeholder="e.g. john_doe"
    ...
    <Transition enter-active-class="transition duration-200" enter-from-class="opacity-0"
      ...
      <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 scale-95"
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-5 sm:p-6 z-10">
            <div class="flex flex-col items-center text-center">
              <div class="w-14 h-14 sm:w-16 sm:h-16 bg-red-50 rounded-2xl flex items-center justify-center mb-4">
                <Trash2 class="w-6 h-6 sm:w-7 sm:h-7 text-red-500" />
              </div>
              <h2 class="text-base sm:text-lg font-bold text-gray-800 mb-1">Delete User</h2>
              <p class="text-sm text-gray-500 mb-1">You are about to permanently delete</p>
              <p class="text-sm font-semibold text-gray-800 bg-gray-100 px-3 py-1 rounded-lg mb-2 max-w-full truncate">
